...
function parseHex(edid) {
	const manufacturerCodeNum = parseInt(edid.slice(16, 20), 16);
	const manufacturerCode = [2, 1, 0].map(n => 31 & (manufacturerCodeNum >> (5 * n))).map(num => String.fromCharCode(charBase + num)).join('');

	const info = { vendor: manufacturerCodeMap[manufacturerCode] || manufacturerCode }

	for (let descStart = 108; descStart != 252; descStart += 36) {
		const descriptor = edid.slice(descStart, descStart + 36);
		if (descriptor.slice(0, 4) === '0000' && descriptor.slice(6, 8).toLowerCase() === 'fc') {
			const model = descriptor.slice(10).match(/[0-9a-f]{2}/gi)
				.map(hexCP437char => parseInt(hexCP437char, 16))
				.map(cp437code => cp437code < 128 ? cp437code : cp437high[cp437code - 128])
				.map(code => String.fromCharCode(code)).join('').trim();

			Object.assign(info, { model })
		}
	}
